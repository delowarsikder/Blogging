  ...
  module V1
    class AuthenticationController < ApplicationController
      ...
      def login
        ...
        if @user && @user.authenticate(params[:user][:password]) # User exist and check password is match
          if @user.confirm? # Check verify email address or not
            token = JsonWebTokenService.encode({ email: @user.email })
            time = Time.now + 1.hours
            render json: { success: true, auth_token: token, token_expiration: time.strftime('%m-%d-%Y %H:%M:%S'), userInfo: @user },
                   status: :ok
          else
